...
import { syncUserToFirestore, adminAuth, adminDb } from '../lib/firebase-admin.ts';
...
// Helper to resolve referral code to a referrer UID
export async function resolveReferrer(rawCode: string | null | undefined): Promise<{ uid: string; referralCode: string; referralType: 'revshare' | 'turnover' } | null> {
  ...
  // 1. Look in SQLite
  try {
    // 1a. Direct referral_code match -> revshare
    let row = await get(
      'SELECT uid, referral_code, turnover_referral_code FROM users WHERE referral_code = ? OR uid = ? OR LOWER(referral_code) = LOWER(?)',
      [cleanCode, cleanCode, cleanCode]
    ) as any;
    if (row && row.uid) {
      return { uid: row.uid, referralCode: row.referral_code || cleanCode, referralType: 'revshare' };
    }

    // 1b. Direct turnover_referral_code match -> turnover
    row = await get(
      'SELECT uid, referral_code, turnover_referral_code FROM users WHERE turnover_referral_code = ? OR LOWER(turnover_referral_code) = LOWER(?)',
      [cleanCode, cleanCode]
    ) as any;
    if (row && row.uid) {
      return { uid: row.uid, referralCode: row.referral_code || cleanCode, referralType: 'turnover' };
    }

    // 1c. If cleanCode is numeric, check if (numericCode - 1) matches a user's referral_code -> turnover
    const numericCode = parseInt(cleanCode, 10);
    if (!isNaN(numericCode) && String(numericCode) === cleanCode) {
      const prevRow = await get(
        'SELECT uid, referral_code, turnover_referral_code FROM users WHERE referral_code = ? OR CAST(referral_code AS INTEGER) = ?',
        [String(numericCode - 1), numericCode - 1]
      ) as any;
      if (prevRow && prevRow.uid) {
        return { uid: prevRow.uid, referralCode: prevRow.referral_code || String(numericCode - 1), referralType: 'turnover' };
      }
    }
  } catch (err) {
    logger.warn(`SQLite referrer query error: ${err}`);
  }

  // 2. Look in Firestore database via adminDb
  if (adminDb) {
    try {
      // 2a. referralCode match -> revshare
      let snap = await adminDb.collection('users').where('referralCode', '==', cleanCode).limit(1).get();
      if (snap && !snap.empty) {
        const d = snap.docs[0];
        return { uid: d.id, referralCode: d.data().referralCode || cleanCode, referralType: 'revshare' };
      }

      // 2b. turnoverReferralCode match -> turnover
      let tSnap = await adminDb.collection('users').where('turnoverReferralCode', '==', cleanCode).limit(1).get();
      if (tSnap && !tSnap.empty) {
        const d = tSnap.docs[0];
        return { uid: d.id, referralCode: d.data().referralCode || cleanCode, referralType: 'turnover' };
      }

      const numericCode = parseInt(cleanCode, 10);
      if (!isNaN(numericCode) && String(numericCode) === cleanCode) {
        // Check affiliateId == numericCode -> revshare
        snap = await adminDb.collection('users').where('affiliateId', '==', numericCode).limit(1).get();
        if (snap && !snap.empty) {
          const d = snap.docs[0];
          return { uid: d.id, referralCode: String(d.data().affiliateId || cleanCode), referralType: 'revshare' };
        }

        // Check affiliateId == numericCode - 1 -> turnover
        snap = await adminDb.collection('users').where('affiliateId', '==', numericCode - 1).limit(1).get();
        if (snap && !snap.empty) {
          const d = snap.docs[0];
          return { uid: d.id, referralCode: String(d.data().affiliateId || String(numericCode - 1)), referralType: 'turnover' };
        }

        snap = await adminDb.collection('users').where('referralCode', '==', String(numericCode - 1)).limit(1).get();
        if (snap && !snap.empty) {
          const d = snap.docs[0];
          return { uid: d.id, referralCode: d.data().referralCode || String(numericCode - 1), referralType: 'turnover' };
        }
      }

      const docSnap = await adminDb.collection('users').doc(cleanCode).get();
      if (docSnap.exists) {
        const dData = docSnap.data();
        return { uid: docSnap.id, referralCode: dData?.referralCode || cleanCode, referralType: 'revshare' };
      }
    } catch (fsErr) {
      logger.warn(`Firestore referrer query error: ${fsErr}`);
    }
  }
...
// Helper to increment referrer stats and sync across SQLite + Firestore
export async function incrementReferrerStats(referrerUid: string, subId?: string) {
  if (!referrerUid) return;

  // 1. Update SQLite
  try {
    await run('UPDATE users SET referral_count = COALESCE(referral_count, 0) + 1 WHERE uid = ?', [referrerUid]);
  } catch (e) {
    logger.warn(`SQLite referral_count increment error: ${e}`);
  }
  ...
  if (adminDb) {
    try {
      ...
      await adminDb.collection('users').doc(referrerUid).update({
        ...
        await adminDb.collection('users').doc(referrerUid).set({
          referralCount: 1
        }, { merge: true });
      });

      if (subId && subId !== 'default' && subId !== 'MAIN') {
        const campSnap = await adminDb.collection('affiliate_campaigns')
          .where('userId', '==', referrerUid)
          .where('subId', '==', subId)
          .limit(1)
          .get();
        if (!campSnap.empty) {
          await campSnap.docs[0].ref.update({
            registrations: FieldValue.increment(1)
          });
        }
      }
    } catch (e) {
      logger.warn(`Firestore referralCount increment error: ${e}`);
    }
  }
}
...
router.post('/register', 
  ...
  try {
    ...
    let turnoverCodeStr = '';
    try {
      const maxRow = await get('SELECT MAX(CASE WHEN CAST(turnover_referral_code AS INTEGER) > CAST(referral_code AS INTEGER) THEN CAST(turnover_referral_code AS INTEGER) ELSE CAST(referral_code AS INTEGER) END) as maxId FROM users') as any;
      const currentMax = maxRow?.maxId ? parseInt(maxRow.maxId) : 100000;
      const baseNum = Math.max(100000, currentMax) + 1;
      affiliateIdStr = String(baseNum);
      turnoverCodeStr = String(baseNum + 1);
    } catch (err) {
      const rnd = 100000 + Math.floor(Math.random() * 899990);
      affiliateIdStr = String(rnd);
      turnoverCodeStr = String(rnd + 1);
    }
    
    let referredBy: string | null = null;
    let resolvedRefCode = '';
    let finalReferralType: string | null = referralType || null;
    if (referralCode) {
      const resolved = await resolveReferrer(referralCode);
      if (resolved) {
        referredBy = resolved.uid;
        resolvedRefCode = resolved.referralCode;
        if (!finalReferralType || finalReferralType === 'revshare') {
          finalReferralType = resolved.referralType || 'revshare';
        }
      }
    }
    ...
    await run(
      `INSERT OR IGNORE INTO users (uid, email, password, referral_code, turnover_referral_code, referred_by_uid, referral_sub_id, referral_type, is_admin) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [uid, email, hashedPassword, affiliateIdStr, turnoverCodeStr, referredBy, referralSubId || null, finalReferralType, isHardcodedAdmin ? 1 : 0]
    );

    if (referredBy) {
      await incrementReferrerStats(referredBy, referralSubId);
    }
    ...
    if (adminDb) {
      try {
        await adminDb.collection('users').doc(uid).set({
          ...
          turnoverReferralCode: turnoverCodeStr,
          referredBy: referredBy || null,
          referredByUid: referredBy || null,
          referredByCode: resolvedRefCode || null,
          referralSubId: referralSubId || null,
          referralType: finalReferralType || null,
          createdAt: new Date()
        }, { merge: true });
      } catch (e) {
    ...
    if (adminDb) {
      await adminDb.collection('users').doc(user.uid).set({
        ...
        referredByCode: resolvedRefCode || referralCode || null,
        referralSubId: referralSubId || null,
...
router.post('/sync', async (req, res) => {
  try {
    ...
    if (user) {
        // Tag referral if user is not yet tagged and referralCode is provided
        if ((!user.referred_by_uid || user.referred_by_uid === '') && referralCode) {
          const resolved = await resolveReferrer(referralCode);
          if (resolved && resolved.uid !== user.uid) {
            await run(
              'UPDATE users SET referred_by_uid = ?, referral_sub_id = COALESCE(referral_sub_id, ?), referral_type = COALESCE(referral_type, ?) WHERE uid = ?',
              [resolved.uid, referralSubId || null, referralType || null, user.uid]
            );
            await incrementReferrerStats(resolved.uid, referralSubId);
            if (adminDb) {
              await adminDb.collection('users').doc(user.uid).set({
                referredBy: resolved.uid,
                referredByUid: resolved.uid,
                referredByCode: resolved.referralCode || referralCode,
                referralSubId: referralSubId || null,
                referralType: referralType || null
              }, { merge: true });
            }
            user = await get('SELECT * FROM users WHERE uid = ?', [user.uid]);
          }
        }
    ...
    if (!user) {
      ...
      if (referralCode) {
        const resolved = await resolveReferrer(referralCode);
        if (resolved && resolved.uid !== firebaseUid) {
          referredBy = resolved.uid;
          resolvedRefCode = resolved.referralCode;
        }
      }
      ...
      if (adminDb) {
        try {
          ...
          if (referredBy) {
            fsData.referredBy = referredBy;
            fsData.referredByUid = referredBy;
            fsData.referredByCode = resolvedRefCode || referralCode;
            if (referralSubId) fsData.referralSubId = referralSubId;
            if (referralType) fsData.referralType = referralType;
          }
          await adminDb.collection('users').doc(firebaseUid).set(fsData, { merge: true });
        } catch (e) {
