...
// Test connection disabled to prevent noisy console errors in restricted environments
/*
...
export const onSnapshot = (ref: any, cb: any, errCb?: any) => {
  ...
  // Poll for changes every 3 seconds to simulate real-time behavior in proxy mode
  const interval = setInterval(() => {
    ref.get().then((s: any) => cb(s)).catch((e: any) => {
