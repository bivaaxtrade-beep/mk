...
export async function syncUserToFirestore(uid: string, data: any) {
  ...
  try {
    ...
    if (existing?.gender && (!cleanData.gender || cleanData.gender === '---' || cleanData.gender === '--')) cleanData.gender = existing.gender;
    if (existing?.dob && (!cleanData.dob || cleanData.dob === 'null')) cleanData.dob = existing.dob;
    if (existing?.birthDay && (!cleanData.birthDay || cleanData.birthDay === '---' || cleanData.birthDay === '--')) cleanData.birthDay = existing.birthDay;
    if (existing?.birthMonth && (!cleanData.birthMonth || cleanData.birthMonth === '---' || cleanData.birthMonth === '--')) cleanData.birthMonth = existing.birthMonth;
    if (existing?.birthYear && (!cleanData.birthYear || cleanData.birthYear === '---' || cleanData.birthYear === '--')) cleanData.birthYear = existing.birthYear;
