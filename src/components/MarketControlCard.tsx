...
export const MarketControlCard = ({ pair, data = {}, updateMarket, appConfig = {}, fetchMarketState, userCurrency = 'BDT' }: any) => {
    ...
    const safeData = data || {};
    const priceNum = typeof safeData.price === 'number' && !isNaN(safeData.price) 
        ? safeData.price 
        : (Number(safeData.price) || 0);
    const formattedPrice = priceNum.toFixed(priceNum < 1 ? 5 : 2);
    const totalUp = Number(safeData.totalUp) || 0;
    const totalDown = Number(safeData.totalDown) || 0;
    const totalVol = totalUp + totalDown;
        ...
        <div className="bg-[#0d0d12] border border-white/5 rounded-[32px] p-6 space-y-5 hover:border-white/10 transition-all group relative overflow-hidden flex flex-col min-h-full">
            ...
            <div className="flex justify-between items-start relative z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-0.5">
                        <h4 className="text-base font-black text-white leading-none flex items-center gap-2">
                            {pair}
                            {safeData.hidden && <span className="px-1.5 py-0.5 rounded bg-red-500/10 text-red-500 text-[8px] border border-red-500/20">HIDDEN</span>}
                        </h4>
                        <div className="flex items-center gap-2 pt-1">
                            <span className="text-[11px] font-mono text-yellow-500 bg-yellow-500/5 px-1.5 py-0.5 rounded border border-yellow-500/10">
                                {getCurrencySymbol(userCurrency)}{formattedPrice}
                            </span>
                            <div className={`px-1.5 py-0.5 rounded text-[9px] font-bold border ${
                                safeData.trend === 'up' ? 'bg-green-500/10 border-green-500/20 text-green-500' : 
                                safeData.trend === 'down' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 
                                'bg-gray-500/10 border-gray-500/20 text-gray-500'
                            }`}>
                                {safeData.trend?.toUpperCase() || 'RANDOM'}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button 
                        onClick={() => updateMarket(pair, { isFrozen: !safeData.isFrozen })}
                        className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${safeData.isFrozen ? 'bg-blue-500 border-blue-500 text-white shadow-lg shadow-blue-500/20' : 'bg-white/5 border-white/5 text-gray-500 hover:text-white hover:bg-white/10'}`}
                        title={safeData.isFrozen ? "Unfreeze Market" : "Freeze Market"}
                    >
                        <Snowflake size={18} />
                    </button>
                </div>
            ...
            <div className="space-y-2 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl p-4">
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-[12px] font-mono font-black text-white bg-yellow-500/15 px-2 py-0.5 rounded border border-yellow-500/20">+{safeData.payout || 82}%</span>
                </div>
                <div className="flex items-center gap-2 pt-1">
                    <input 
                        type="range"
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2 border border-white/5 focus-within:border-yellow-500/50 transition-colors">
                        ...
                        <input 
                            type="number" 
                            value={safeData.minTrade || appConfig?.globalMinTrade || 100}
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2 border border-white/5 focus-within:border-yellow-500/50 transition-colors">
                        ...
                        <input 
                            type="number" 
                            value={safeData.maxTrade || appConfig?.globalMaxTrade || 50000}
                    ...
                    >
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 space-y-3">
                            <div className="flex justify-between items-center">
                                ...
                                <span className={`text-[11px] font-black font-mono ${(safeData.pressure || 0) > 0 ? 'text-green-500' : (safeData.pressure || 0) < 0 ? 'text-red-500' : 'text-gray-400'}`}>
                                    {(safeData.pressure || 0) > 0 ? '+' : ''}{safeData.pressure || 0}%
                                </span>
                            </div>
                            <input 
                                type="range"
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    type="number" 
                                    step="0.00001"
                                    placeholder={priceNum.toFixed(5)}
                                    onKeyPress={(e: any) => {
                                        if (e.key === 'Enter') {
                        ...
                        <div className="space-y-2">
                            ...
                            <select 
                                value={safeData.manipulation?.mode || 'percentage'}
                                onChange={e => fetch('/api/admin/market/manipulation', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ 
                                        pair, 
                                        targetTrend: safeData.manipulation?.targetTrend || 'random', 
                                        profitPercentage: safeData.manipulation?.profitPercentage || 0, 
                                        enabled: safeData.manipulation?.enabled || false,
                                        mode: e.target.value
                                    })
                        ...
                        <div className="grid grid-cols-2 gap-3">
                             <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-[9px] font-black text-yellow-500">{safeData.houseEdge || 10}%</span>
                                </div>
                                <input 
                                    type="number"
                                    placeholder="Set %"
                             ...
                             <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex flex-col justify-between">
                                ...
                                <div className="flex items-center justify-between mt-2">
                                    <span className="text-[9px] font-medium text-gray-400">{safeData.hidden ? 'Hidden' : 'Visible'}</span>
                                    <button 
                                        onClick={() => updateMarket(pair, { hidden: !safeData.hidden })}
                                        className={`w-8 h-4 rounded-full transition-all relative flex items-center px-0.5 ${safeData.hidden ? 'bg-red-500' : 'bg-green-500'}`}
                                    >
                                        <div className={`w-3 h-3 rounded-full bg-white shadow-sm transition-all ${safeData.hidden ? 'ml-auto' : 'mr-auto'}`} />
                                    </button>
                                </div>
                             </div>
                        </div>
                        ...
                        <div className="space-y-3 p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                            <div className="flex justify-between items-center text-[9px] font-black text-gray-500 uppercase tracking-widest">
                                <span>Volume UP</span>
                                <span className="text-green-500 font-mono text-[10px]">{getCurrencySymbol(userCurrency)}{totalUp.toLocaleString()}</span>
                            </div>
                            <div className="w-full h-1 bg-gray-900 rounded-full overflow-hidden">
                                <div 
                                    className="h-full bg-green-500 transition-all duration-500" 
                                    style={{ width: `${totalVol > 0 ? (totalUp / totalVol) * 100 : 50}%` }}
                                />
                            </div>
                            <div className="flex justify-between items-center text-[9px] font-black text-gray-500 uppercase tracking-widest">
                                <span>Volume DOWN</span>
                                <span className="text-red-500 font-mono text-[10px]">{getCurrencySymbol(userCurrency)}{totalDown.toLocaleString()}</span>
                            </div>
                            <button 
                                onClick={() => {
                                    ...
                                    {['volatility', 'maintenance'].map((reason) => (
                                        <button
                                            ...
                                            className={`py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                                                (safeData.freezeReason || 'volatility') === reason 
