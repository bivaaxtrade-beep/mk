...
interface Drawing {
  id: string;
  type: 'line' | 'horizontal' | 'vertical' | 'rectangle' | 'trend' | 'fibo' | 'ray' | 'channel' | 'extended_line' | 'fibo_fan' | 'pitchfork' | 'triangle' | 'arrow' | 'measure' | 'text';
  points: Point[];
  color: string;
  width: number;
  assetId?: string;
...
export const DrawingOverlay = React.memo(({
  ...
  const handleMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (selectedTool === 'horizontal' || selectedTool === 'vertical' || selectedTool === 'text') {
        const newDrawing: Drawing = {
            ...
            width: 2,
            assetId: activeAsset,
            label: selectedTool === 'text' ? 'Analysis Zone' : undefined
        };
        setDrawings([...drawings, newDrawing]);
        setSelectedTool(null);
    } else if (selectedTool === 'triangle' || selectedTool === 'pitchfork') {
        const newDrawing: Drawing = {
            id: Math.random().toString(36).substr(2, 9),
            type: selectedTool as any,
            points: [newPoint, newPoint, newPoint],
            color: '#FFE24C',
            width: 2,
            assetId: activeAsset,
        };
  ...
  const renderDrawing = (drawing: Drawing, isGhost = false) => {
    ...
    if (drawing.type === 'horizontal') {
        ...
        const dx = x2 - x1;
        const dy = y2 - y1;
        const extX1 = x1 - dx * 50;
        const extY1 = y1 - dy * 50;
        const extX2 = x1 + dx * 50;
        const extY2 = y1 + dy * 50;
        element = (
            <>
                <line x1={extX1} y1={extY1} x2={extX2} y2={extY2} {...style} />
                <line x1={extX1} y1={extY1} x2={extX2} y2={extY2} stroke="transparent" strokeWidth={hitAreaWidth} onContextMenu={(e) => handleRightClick(e, drawing.id)} onMouseDown={(e) => handleMouseDownOnBody(e, drawing.id)} cursor={isEditing ? "move" : "default"} />
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        );
        ...
        element = (
            <>
                ...
                <polygon points={`${x2},${y2} ${arrowP1X},${arrowP1Y} ${arrowP2X},${arrowP2Y}`} fill={drawing.color} opacity={isGhost ? 0.5 : 1} />
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="transparent" strokeWidth={hitAreaWidth} onContextMenu={(e) => handleRightClick(e, drawing.id)} onMouseDown={(e) => handleMouseDownOnBody(e, drawing.id)} cursor={isEditing ? "move" : "default"} />
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        ...
        element = (
            <>
                <line x1={x1} y1={y1} x2={x2} y2={y2} {...style} />
                <line x1={x1} y1={y1 - offsetY} x2={x2} y2={y2 - offsetY} {...style} strokeDasharray="4" />
                <polygon points={`${x1},${y1} ${x2},${y2} ${x2},${y2 - offsetY} ${x1},${y1 - offsetY}`} fill={drawing.color} fillOpacity={0.12} stroke="none" />
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        );
    } else if (drawing.type === 'fibo_fan') {
        const x1 = points[0].x!;
        const y1 = points[0].y!;
        const x2 = points[1].x!;
        const y2 = points[1].y!;
        const dy = y2 - y1;
        const fan382 = y1 + dy * 0.382;
        const fan500 = y1 + dy * 0.5;
        const fan618 = y1 + dy * 0.618;
        element = (
            <>
                <line x1={x1} y1={y1} x2={x2} y2={y2} {...style} strokeDasharray="2" opacity={0.6} />
                <line x1={x1} y1={y1} x2={x2} y2={fan382} {...style} />
                <text x={x2 + 5} y={fan382} fill={drawing.color} fontSize="9" fontWeight="bold">38.2%</text>
                <line x1={x1} y1={y1} x2={x2} y2={fan500} {...style} />
                <text x={x2 + 5} y={fan500} fill={drawing.color} fontSize="9" fontWeight="bold">50.0%</text>
                <line x1={x1} y1={y1} x2={x2} y2={fan618} {...style} />
                <text x={x2 + 5} y={fan618} fill={drawing.color} fontSize="9" fontWeight="bold">61.8%</text>
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        ...
        const midX = (x2 + x3) / 2;
        const midY = (y2 + y3) / 2;
        const medianDX = (midX - x1) * 3;
        const medianDY = (midY - y1) * 3;
        element = (
            <>
                <line x1={x2} y1={y2} x2={x3} y2={y3} {...style} strokeDasharray="3" />
                <line x1={x1} y1={y1} x2={x1 + medianDX} y2={y1 + medianDY} {...style} strokeWidth={2} />
                <line x1={x2} y1={y2} x2={x2 + medianDX} y2={y2 + medianDY} {...style} opacity={0.8} />
                <line x1={x3} y1={y3} x2={x3 + medianDX} y2={y3 + medianDY} {...style} opacity={0.8} />
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        ...
        element = (
            <>
                <polygon points={pts} {...style} fill={drawing.color} fillOpacity={0.15} />
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
                ))}
            </>
        ...
        element = (
            <>
                <rect x={minX} y={minY} width={w} height={h} fill={isPos ? "#00c98020" : "#f45c5c20"} stroke={isPos ? "#00c980" : "#f45c5c"} strokeWidth={1} strokeDasharray="3" />
                <g transform={`translate(${minX + w / 2 - 45}, ${minY + h / 2 - 12})`}>
                    <rect width="90" height="24" rx="6" fill="#1e2026" stroke={isPos ? "#00c980" : "#f45c5c"} strokeWidth={1} />
                    <text x="45" y="15" fill={isPos ? "#00c980" : "#f45c5c"} fontSize="10" fontWeight="bold" textAnchor="middle">
                        {isPos ? '+' : ''}{diff.toFixed(5)} ({pct}%)
                    </text>
                </g>
                {isEditing && points.map((p, i) => (
                    <circle key={i} cx={p.x!} cy={p.y!} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, i)} />
        ...
        element = (
            <>
                <g transform={`translate(${x}, ${y})`}>
                    <rect x="-5" y="-14" width="110" height="22" rx="4" fill="#2a2c33" stroke={drawing.color} strokeWidth={1.5} />
                    <text x="5" y="1" fill="#FFE24C" fontSize="11" fontWeight="extrabold">
                        {drawing.label || "Analysis Level"}
                    </text>
                </g>
                {isEditing && <circle cx={x} cy={y} r={6} fill={drawing.color} stroke="white" strokeWidth={2} cursor="move" onMouseDown={(e) => handleMouseDownOnHandle(e, drawing.id, 0)} />}
            </>
        );
    }
