...
const RocketDeposit: React.FC = () => {
  ...
  const agentType = methodData?.agentType || appConfig.rocket_type || "Agent";

  const t = {
    timeLeft: "Time Remaining",
    submitted: "Request Submitted!",
    successDesc: `Your Rocket manual deposit request has been received. Our support team will verify the transaction with your ledger records shortly.`,
    orderId: "Order ID",
    amount: "Amount",
    return: "Return to Terminal",
    warning: "Please do not change the deposit amount",
    step1: "Copy Cash Out Number",
    step1Desc: appConfig.rocket_step1_en || `Send money to the number below using Rocket cash out.`,
    ...
    guideTitle: `How to Deposit via Rocket`,
    guide: appConfig.rocket_guide_en ? appConfig.rocket_guide_en.split('\n') : [
      "1. Open your Rocket app or dial USSD (*322#).",
      "2. Choose the Cash Out option.",
      "3. Enter or paste the copied Rocket account number.",
      "4. Enter the exact payment amount shown above.",
      "5. Complete the cash out by entering your Rocket PIN.",
    ...
    try {
      if (auth.currentUser) {
        ...
        await addDoc(collection(db, `users/${auth.currentUser.uid}/transactions`), {
          ...
          bonusPercentage: bonus ? Number(bonus) : 0,
          bonusAmount: bonus ? (Number(amount) * Number(bonus)) / 100 : 0,
          timestamp: Date.now(),
          category: 'MFS'
        });
      ...
      setIsSuccess(true);
      toast.success("Transaction submitted for support verification");
  ...
  return (
    <div className="min-h-screen bg-[#0C0D12] text-gray-100 font-sans flex flex-col items-center">
      <SEO title="Rocket Deposit" description="Manage your Rocket Deposit on Bivaax Trade Platform." />
      ...
      <div className="w-full pt-6 pb-2 flex flex-col items-center relative overflow-hidden bg-gradient-to-b from-[#1C1D24] to-[#0C0D12]">
          <div className="w-full max-w-md px-6 flex items-center justify-between text-white mb-4 z-10">
            ...
            <div className="flex gap-1.5">
                <span className="px-2.5 py-1 rounded-md text-[11px] font-extrabold bg-[#FFE24C] text-black">
                  EN
                </span>
            </div>
          </div>
          ...
          <div className="w-full max-w-md px-6 flex items-center justify-start gap-4 z-10 mb-6 mt-2">
              ...
              <div>
                <h1 className="text-xl font-black tracking-tight text-white uppercase">Rocket Manual P2P</h1>
          ...
          <div className="w-full max-w-md px-4 mb-4 z-10">
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 text-[11px] text-yellow-500 leading-relaxed">
              <strong>Disclaimer:</strong> This manual payment system is independent and NOT affiliated with or endorsed by Rocket MFS/DBBL. Send money manually and enter your transaction ID.
            </div>
          </div>
                  ...
                  <div className="px-4 sm:px-5 pb-5">
                      <div className="bg-black/30 border border-white/5 rounded-2xl p-4 flex items-center justify-between group">
                          <div className="flex items-center gap-3 min-w-0">
                              <div className="relative w-8 h-8 bg-purple-500/10 rounded-full flex items-center justify-center text-purple-500 font-black text-xs shrink-0">
                                 Ro
                              </div>
