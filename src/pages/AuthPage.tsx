...
export default function AuthPage() {
  ...
  const isForgotPasswordPath = location.pathname === '/forgot-password' || location.pathname === '/reset-password';
  
  const [view, setView] = useState<'login' | 'register' | 'forgot_password' | 'verify_otp' | 'reset_password'>(
    isRegisterPath ? 'register' : (isForgotPasswordPath ? 'forgot_password' : 'login')
  );
  ...
  useEffect(() => {
    if (urlRef) {
      ...
      import('../lib/affiliate').then(({ getUserByAffiliateId }) => {
        getUserByAffiliateId(urlRef).then((resolved) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'forgot_password') {
        ...
        if (finalReferrerUid) {
          userData.referredBy = finalReferrerUid;
          userData.referredByUid = finalReferrerUid;
          userData.referredByCode = ref;
        }
        if (sub) {
          userData.referralSubId = sub;
          userData.referredSub = sub;
        }
        if (type) {
          userData.referralType = type;
        }

        await setDoc(doc(db, 'users', user.uid), userData, { merge: true });

        // Increment referral count for referrer in Firestore
        if (finalReferrerUid) {
          const referrerRef = doc(db, 'users', finalReferrerUid);
          await updateDoc(referrerRef, {
            referralCount: increment(1)
          }).catch(e => console.error("Failed to increment referral count:", e));
        }
