...
import { 
  ...
  Check, X, ChevronRight, Bell, Moon, Sun, Download, Trash2, Edit3, Eraser, RotateCcw,
...
export default function EnterpriseSupportCenter() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot: any) => {
      ...
      snapshot.forEach((doc: any) => {
        const data = doc.data();
        // Soft-delete filter for admin
        if (!data.hiddenFromAdmin && !data.dismissedByAdmin && !data.deletedByAdmin) {
          ticketsData.push({ id: doc.id, ...data } as TicketItem);
        }
      });
    ...
    const unsubCanned = onSnapshot(collection(db, 'canned_responses'), (snap: any) => {
      ...
      if (snap.empty && items.length === 0) {
        const initialCanned = [
          ...
          { title: 'Deposit Delay', text: 'We are experiencing a slight delay with TRC-20 nodes. Please wait 15 mins.', category: 'Payment', shortcut: 'delay', createdAt: Date.now() }
        ];
  ...
  useEffect(() => {
    ...
    const unsubMsg = onSnapshot(msgQuery, (snapshot: any) => {
      ...
      const clearedAt = (selectedTicket as any).adminClearedAt || 0;

      snapshot.forEach((doc: any) => {
        const data = doc.data();
        // Soft-delete filter for admin
        if (data.hiddenFromAdmin || data.deletedForAdmin) return;
        
        const msgTime = data.createdAt || 0;
        if (clearedAt && msgTime < clearedAt) return;

        msgsData.push({ id: doc.id, ...data } as MessageItem);
      });
    ...
    const getAiSuggestion = async () => {
      if (!selectedTicket || selectedTicket.status === 'Resolved' || selectedTicket.status === 'Resolved' as any) return;
      setIsAiLoading(true);
      try {
  ...
  const dismissTicketForAdmin = async (ticketId: string) => {
    if (!window.confirm("টিকিটটি কি অ্যাডমিন প্যানেল থেকে মুছে ফেলতে চান? (ইউজারের ডিভাইসে হিস্ট্রি বজায় থাকবে)")) return;
    try {
      ...
      toast.success("টিকিটটি মুছে ফেলা হয়েছে");
    } catch (err) {
      toast.error("মুছে ফেলতে ব্যর্থ হয়েছে");
    }
  };

  const deleteMessageForAdmin = async (ticketId: string, messageId: string) => {
  ...
  const clearChatHistoryForAdmin = async (ticketId: string) => {
    if (!window.confirm("এই চ্যাট হিস্ট্রি কি অ্যাডমিন প্যানেল থেকে ক্লিয়ার করতে চান?")) return;
    try {
      await updateDoc(doc(db, 'tickets', ticketId), {
        adminClearedAt: Date.now()
      });
      setMessages([]);
      toast.success("হিস্ট্রি ক্লিয়ার করা হয়েছে");
    } catch (err) {
      toast.error("ক্লিয়ার করা যায়নি");
    }
  };
                  ...
                  <>
                    ...
                    <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-[#12161c]">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          onClick={() => clearChatHistoryForAdmin(selectedTicket.id)}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                      ...
                      {messages.map(m => (
                        <div key={m.id} className={`flex flex-col group/msg ${m.senderType === 'agent' ? 'items-end' : 'items-start'}`}>
                          ...
                          <div className="flex items-center gap-2 max-w-[80%] relative">
                            {m.senderType === 'agent' && (
                              <button 
                                onClick={() => deleteMessageForAdmin(selectedTicket.id, m.id)}
                                className="p-1 rounded bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all opacity-0 group-hover/msg:opacity-100"
                                title="Delete for Admin"
                              >
                                <Trash2 size={12} />
                              </button>
                            )}
